    ...
    override init() {
        ...
        func generateFrequencies(frame_count: Int32) -> [Float32] {
            ...
            combined_frequency_samples.append(contentsOf: frame_indicies.0.enumerated().map({ kv in
                ...
                let amplitude_ : Float32 = scale(min_new: Float32.zero, max_new: amplitude, val_old: pow(cos(((Float32.pi * frame_indicies.1[kv.offset]))), 2.0), min_old: Float32.zero, max_old: cos(tau * pow(amplitude, 0.333)))

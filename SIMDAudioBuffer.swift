...
@objc class SIMDAudioBuffer: NSObject {
...
//    public func createAudioBuffer(withFormat audioFormat: AVAudioFormat, completionBlock: CreateAudioBufferCompletionBlock) {
    public func createAudioBuffer(withFormat audioFormat: AVAudioFormat) -> AVAudioPCMBuffer {
//        let tau: simd_float1 = simd_float1_
        ...
        let phaseAngularUnits = simd_double2x2(rows: [
            simd_double2(M_PI_SQR / Double(splitFrame), M_PI_SQR / (simd_double1(frameCount) - Double(splitFrame))),
            simd_double2(M_PI_SQR / (simd_double1(frameCount) - Double(splitFrame)), M_PI_SQR / Double(splitFrame))
        ])
        
        // Assuming 'frequencies' and 'thetaIncrements' are defined earlier
        let thetaIncrements = phaseAngularUnits * frequencies[0]
...
//    convenience init(audioFormat: AVAudioFormat, audioFrameCount: AVAudioFrameCount, audioBufferList: inout UnsafeMutablePointer<AudioBufferList>) {
    convenience init(audioFormat: AVAudioFormat, audioFrameCount: AVAudioFrameCount) {
        self.init()
